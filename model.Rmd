...
init_inter2 <- init.icm(s.num = 49,
                       i.num = 1,
                       r.num = 0)

control_inter2 <- control.icm(
  type = "SIR",
  nsims = 100,
  nsteps = sim_days,
  verbose = T
)
mod_inter2 <- quiet(icm(param_inter2, init_inter2, control_inter2))
```
...
Prevalence of compartments
```{r}
plot(mod_inter2, propfrac = T)
```
```{r}
available_workers_inter2 <-
  apply(mod_inter2$epi$s.num, 1, median) + apply(mod_inter2$epi$r.num, 1, median)
...
daily_prodictivity <- tibble(
  ...
  productivity_inter = available_workers_inter * (34125 /
                                                    30),
  productivity_inter2 = available_workers_inter2 * (34125 /
                                                    30)
)
...
```{r}
daily_prodictivity %>% pivot_longer(-c(day, saved)) %>%  ggplot(aes(day, value, col= name)) +
  geom_line(size= 1.2) +
