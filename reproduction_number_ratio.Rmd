...
a<-tibble(date = df_cum$date, positive = diff(c(0, df_cum$positive)), tested = diff(c(0, raw$numberOfTestedGraph %>% filter(date >= as.Date("2020-03-01")) %>% dplyr::select(value) %>% pull, NA)))
        
a$dif <- a$positive/a$tested

a %>% ggplot(aes(date, dif)) + geom_line() + geom_smooth()
